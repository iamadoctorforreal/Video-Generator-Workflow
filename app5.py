...
def sanitize_text_for_tts(text):
    """Fix problematic words and characters for Kokoro TTS"""
    # 1. CRASH PREVENTION: Remove ellipses/extra dots and non-ASCII (emojis)
    text = text.replace("....", ".").replace("...", ".").replace("..", ".")
    text = text.encode("ascii", "ignore").decode()
    
    # 2. PRONUNCIATION: Replace Japanese names
    replacements = {
        'Aiko': 'Eye ko',
        'Haru': 'Ha roo',
        'Yuki': 'You kee',
...
def create_caption(text, clip_size, duration):
    ...
    try:
        ...
        if txt is None:
            print(f"   ⚠️  No fonts available, skipping captions")
            return None
