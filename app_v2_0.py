...
def create_dynamic_captions(words, clip_size):
    word_clips = []
    for w in words:
        # Create a small, popping clip for EACH word
        duration = w['end'] - w['start']
        if duration <= 0: continue
        
        try:
            word_clip = TextClip(
                text=w['word'].upper(), # TikTok style is often UPPERCASE
                font=r'C:\Windows\Fonts\arialbd.ttf',
                font_size=90,
                color='yellow',
                stroke_color='black',
                stroke_width=3,
                method='label' # 'label' fits the box to the text
            ).with_start(w['start']).with_duration(duration).with_position('center')
            
            # Add a tiny "pop" animation
           # word_clip = word_clip.resized(lambda t: 1 + 0.1 * np.sin(np.pi * t / duration))

            #If I want an aggresive tiktok styled caption I will enable the ine below
            # Instead of sin wave, use a linear "Overshoot"
            # 0% of duration = 1.3x size, 20% of duration = 1.0x size
            word_clip = word_clip.resized(lambda t: 1.3 - 0.3 * (t / (duration * 0.2)) if t < (duration * 0.2) else 1.0)
