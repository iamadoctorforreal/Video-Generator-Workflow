...
def create_dynamic_captions(words, clip_size, active_index=0, caption_position="bottom"):
    ...
    FONT_SIZE = 88
    GAP = 14
    STEPS = 6  # steps to simulate opacity easing

    def ease_out(progress):
        return 1 - (1 - progress) ** 3

    # Build pairs: (word_A, word_B, start, end)
    # word_A is active first, then word_B becomes active
    pairs = []
    i = 0
    while i < len(words):
        word_a = words[i]
        word_b = words[i + 1] if i + 1 < len(words) else None
        pairs.append((word_a, word_b))
        i += 2  # jump by 2 — new pair every 2 words

    for pair_idx, (word_a, word_b) in enumerate(pairs):
        # Pair is visible from word_a start → word_b end (or word_a end if solo)
        pair_start = word_a['start']
        pair_end = word_b['end'] if word_b else word_a['end']
        pair_duration = pair_end - pair_start
        if pair_duration <= 0:
            continue

        text_a = word_a['word'].upper()
        text_b = word_b['word'].upper() if word_b else None

        try:
            # Measure sizes
            clip_a_measure = TextClip(text=text_a, font=FONT, font_size=FONT_SIZE, method='label', margin=(30, 30))
            a_w, a_h = clip_a_measure.w, clip_a_measure.h

            if text_b:
                clip_b_measure = TextClip(text=text_b, font=FONT, font_size=FONT_SIZE, method='label', margin=(30, 30))
                b_w, b_h = clip_b_measure.w, clip_b_measure.h
            else:
                b_w, b_h = 0, a_h

            total_w = a_w + GAP + b_w if text_b else a_w
            start_x = (w - total_w) // 2
            a_x = start_x
            b_x = start_x + a_w + GAP
            a_y = int(y_pos - a_h // 2)
            ...
            # ── Phase 1: word_a is ACTIVE (yellow), word_b is WHITE ──
            # word_a active duration
            a_duration = (word_a['end'] - word_a['start'])
            # Easing window at end of word_a: yellow→white for a, white→yellow for b
            EASE_DUR = min(0.08, a_duration * 0.3)
            a_stable_dur = a_duration - EASE_DUR

            if a_stable_dur > 0:
                # word_a: yellow, stable
                word_clips.append(
                    TextClip(text=text_a, font=FONT, font_size=FONT_SIZE,
                             color='yellow', stroke_color='black', stroke_width=4,
                             method='label', margin=(30, 30))
                    .with_opacity(1.0)
                    .with_position((a_x, a_y))
                    .with_start(pair_start)
                    .with_duration(a_stable_dur)
                )
                if text_b:
                    # word_b: white, stable
                    word_clips.append(
                        TextClip(text=text_b, font=FONT, font_size=FONT_SIZE,
                                 color='white', stroke_color='black', stroke_width=3,
                                 method='label', margin=(30, 30))
                        .with_opacity(0.8)
                        .with_position((b_x, b_y))
                        .with_start(pair_start)
                        .with_duration(a_stable_dur)
                    )

            # Ease transition: a goes yellow→white, b goes white→yellow
            if text_b:
                ease_start = pair_start + a_stable_dur
                for step in range(STEPS):
                    p = ease_out((step + 1) / STEPS)
                    s = ease_start + (step / STEPS) * EASE_DUR
                    d = EASE_DUR / STEPS

                    # a fading from yellow to white — use opacity trick: fade yellow out, white in
                    # We render word_a twice: yellow fading out, white fading in
                    word_clips.append(
                        TextClip(text=text_a, font=FONT, font_size=FONT_SIZE,
                                 color='yellow', stroke_color='black', stroke_width=4,
                                 method='label', margin=(30, 30))
                    ...
                    # b going white→yellow
                    word_clips.append(
                        TextClip(text=text_b, font=FONT, font_size=FONT_SIZE,
                                 color='white', stroke_color='black', stroke_width=3,
                                 method='label', margin=(30, 30))
                        .with_opacity(0.8 * (1.0 - p))
                        .with_position((b_x, b_y))
                        .with_start(s).with_duration(d)
                    )
                    word_clips.append(
                ...
                # ── Phase 2: word_b is ACTIVE (yellow), word_a is WHITE ──
                b_start = word_b['start']
                b_end = word_b['end']
                b_stable_start = ease_start + EASE_DUR
                b_stable_dur = b_end - b_stable_start
                if b_stable_dur > 0:
                    word_clips.append(
                        TextClip(text=text_a, font=FONT, font_size=FONT_SIZE,
                                 color='white', stroke_color='black', stroke_width=3,
                        ...
                        .with_opacity(0.8)
                        .with_position((a_x, a_y))
                        .with_start(b_stable_start)
                        .with_duration(b_stable_dur)
                    )
                    word_clips.append(
                        TextClip(text=text_b, font=FONT, font_size=FONT_SIZE,
                                 color='yellow', stroke_color='black', stroke_width=4,
       ...
       #MUSIC_PATH = os.path.join(os.getcwd(), "bg_music.mp3")
        ...
        for i, scene in enumerate(request.scenes, 1):
            ...
            if not media_path or not os.path.exists(media_path):
                clip = ColorClip(size=(720, 1280), color=(30,30,30)).with_duration(audio_clip.duration)
            else:
            ...
            if request.add_effects: clip = apply_pan_zoom_effect(clip)
                ...
                # Replace old static caption with the new word-level clips
            #   word_clips = create_dynamic_captions(word_data, (clip.w, clip.h))
            #   if word_clips:
            ...
            if request.add_captions:
                word_clips = create_dynamic_captions(
                    word_data, 
                   (clip.w, clip.h),
                    active_index=0,  # not used directly now, kept for signature compat
